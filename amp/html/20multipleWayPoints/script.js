...
function setUp(){
  for (var i = 0; i < countPoints; i++) {
    ...
    A.point = new Point(A.pos.dx,A.pos.dy,10,"lightgreen");
    points.push(A);
  }
  ...
  for (var i = 0; i < playercount; i++) {
    ...
    player.point = new Point(player.pos.dx,player.pos.dy,5,"pink");
    spelers.push(player);
  }
...
for (var i = 0; i < playercount; i++) {
  ...
  if(moveToA){
    distance = spelers[i].point.distanceTOP(points[spelers[i].state].point);
    spelers[i].vel.differenceVector(points[spelers[i].state].pos,spelers[i].pos);
    if(spelers[i].point.distanceTOP(points[spelers[i].state].point)<5){
      moveToA = false;
      spelers[i].state++;
    }
  } else {
